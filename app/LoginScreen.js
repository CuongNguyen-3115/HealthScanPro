...
const LoginScreen = () => {
  ...
  const { login, hasHealthProfile } = useUser();
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      if (result.success) {
        // Kiểm tra xem user đã có hồ sơ sức khỏe chưa
        if (!hasHealthProfile()) {
          Alert.alert(
            'Chào mừng!', 
            'Bạn chưa có hồ sơ sức khỏe. Hãy tạo hồ sơ để sử dụng đầy đủ tính năng của ứng dụng.',
            [
              { 
                text: 'Tạo hồ sơ', 
