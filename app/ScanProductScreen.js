...
const ScanProductScreen = () => {
  ...
  // Kiểm tra trạng thái quyền camera khi component load
  React.useEffect(() => {
    if (permission) {
      if (permission.granted) {
        ...
        setCameraPermissionStatus('not-asked');
      }
    }
  ...
  const openAppSettings = async () => {
    try {
      ...
      Alert.alert(
        ...
        'Vui lòng vào Cài đặt > Quyền riêng tư > Camera để cấp quyền cho ứng dụng.',
  ...
  const handleOpenCamera = async () => {
    try {
      ...
      if (!permission) {
        console.log('Chưa có quyền camera, đang yêu cầu...');
        const permissionResponse = await requestPermission();
        if (!permissionResponse.granted) {
          Alert.alert(
            'Cần quyền truy cập camera', 
            'Ứng dụng cần quyền truy cập camera để chụp ảnh sản phẩm. Vui lòng vào Cài đặt để cấp quyền.',
            [
      ...
      // Kiểm tra lại quyền sau khi request
      if (permission && permission.granted) {
        console.log('Có quyền camera, mở camera...');
      ...
      console.error('Lỗi khi xử lý quyền camera:', error);
      Alert.alert(
        'Lỗi', 
        'Có lỗi xảy ra khi yêu cầu quyền camera. Vui lòng thử lại.',
        [{ text: 'OK', style: 'default' }]
      );
    }
  };
  ...
  const handleSelectFromLibrary = async () => {
    try {
      // Mở thư viện ảnh trực tiếp mà không yêu cầu quyền
  ...
  const handleAnalyzeImage = async () => {
    ...
    try {
      ...
      // Gửi request lên API
      const response = await fetch(`${API_BASE_URL}/label/analyze`, {
        method: 'POST',
        body: formData,
        headers: {
      ...
      if (result.ok) {
        // Chuyển đến màn hình kết quả phân tích
        router.push({
          pathname: '/ProductAnalysisScreen',
          params: {
            analysisResult: JSON.stringify(result.label),
        ...
        Alert.alert(
          'Lỗi phân tích',
          result.error || 'Không thể phân tích ảnh sản phẩm. Vui lòng thử lại.'
        );
      }
    } catch (error) {
      ...
      Alert.alert(
        'Lỗi',
        'Có lỗi xảy ra khi phân tích ảnh. Vui lòng kiểm tra kết nối mạng và thử lại.'
      );
    } finally {
              ...
              {capturedImage ? (
                <View style={styles.imagePreviewContainer}>
                  ...
                  <View style={styles.imageActions}>
                    ...
                    <TouchableOpacity style={styles.libraryRetakeButton} onPress={handleSelectFromLibrary}>
                      <Ionicons name="images" size={20} color="#3b82f6" />
                      <Text style={styles.libraryRetakeText}>Chọn khác</Text>
                    </TouchableOpacity>
                    <TouchableOpacity 
                ...
                <>
                  ...
                  <TouchableOpacity 
                    style={[
                      ...
                      cameraPermissionStatus === 'denied' && styles.openCameraButtonDisabled
                    ]} 
                    onPress={handleOpenCamera}
                    disabled={cameraPermissionStatus === 'denied'}
                  >
                    <Ionicons 
                      name={cameraPermissionStatus === 'denied' ? 'camera-off' : 'camera'} 
                      size={24} 
                      color="white" 
                    />
                    <Text style={styles.openCameraText}>
                      {cameraPermissionStatus === 'denied' ? 'Cần cấp quyền camera' : 'Mở Camera'}
                    </Text>
                  </TouchableOpacity>
          ...
          {selectedMode === 'upload' && (
            <View style={styles.uploadCard}>
              ...
              {capturedImage ? (
                ...
                <TouchableOpacity style={styles.uploadButton} onPress={handleSelectFromLibrary}>
