# -*- coding: utf-8 -*-
"""
HealthScan Authentication API - MongoDB Integration
Tích hợp authentication với MongoDB cho HealthScan Pro
"""
...
CORS(auth_app, resources={r"/*": {"origins": "*"}})
...
@auth_app.route('/api/health', methods=['GET'])
def health_check():
...
@auth_app.route('/api/auth/verify', methods=['GET'])
@token_required
...
@auth_app.route('/api/auth/users/<user_id>', methods=['PUT'])
@token_required
...
@auth_app.route('/api/auth/create-admin', methods=['POST'])
def create_admin():
...
if __name__ == '__main__':
    print("[INFO] Starting HealthScan Authentication API...")
    print(f"[INFO] MongoDB URI: {MONGODB_URI}")
    print(f"[INFO] Database: {DATABASE_NAME}")
    print(f"[INFO] JWT Secret: {'*' * len(JWT_SECRET)}")
    
    # Run on port 8889 to avoid conflict with main server
    auth_app.run(host='0.0.0.0', port=8889, debug=True)
